...
breedSelect.addEventListener('change', async function(event) {
  ...
  // Clear existing content in carousel and infoDump
  carousel.innerHTML = '';
  infoDump.innerHTML = '';
  ...
  try {
      ...
      breedName.textContent = `Breed Name: ${data[0].breeds[0].name}`;
      infoDump.appendChild(breedName);

      const breedDescription = document.createElement('p');
      breedDescription.textContent = `Description: ${data[0].breeds[0].description}`;
